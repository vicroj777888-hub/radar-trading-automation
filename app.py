...
# ==========================================
# ESCRITURA SEGURA POR FUSION (NUNCA BORRA FILAS)
# ==========================================
...
def escribir_sim(ws, filas):
    ...
    for intento in (1, 2, 3):
        try:
            ...
            fusion = fusionar_filas(existentes, filas)
...
def validar_horario_manual(estrategia, ahora):
    ...
    if estrategia == 'Primera Vela Roja':
        if not (h == 10 and m <= 5):
            return False, 'PVR solo es valida entre 10:00 y 10:05'
        return True, 'horario valido'
    if estrategia in ('Hanger en Diario', 'Primer Gap al Alza'):
        if not (h >= 15 and m >= 55):
            return False, 'Hanger/Primer Gap solo cerca del cierre (15:55+)'
        return True, 'horario valido'
    if h < 11:
        return False, 'estrategias generales solo desde las 11:00'
    return True, 'horario valido'

# ==========================================
# ESTADISTICAS POR ESTRATEGIA (n, win rate, EM)
# ==========================================

def calcular_estadisticas(cerradas):
    stats = {}
    for f in cerradas:
        est = str(f.get('Estrategia', 'Sin Estrategia'))
        notas = str(f.get('Notas', ''))
        nom = str(f.get('NOM', ''))
        if nom == 'MANUAL':
            cubo = 'Manual (revision aparte)'
        elif ('entrada valida' in notas and '+' not in est and
              'venta tardia' not in notas and 'vencida' not in notas):
            cubo = 'Limpia (v15+)'
        else:
            cubo = 'Historica / Contaminada'
        if est not in stats:
            stats[est] = {
                'Limpia (v15+)': {'n': 0, 'w': 0, 'l': 0, 'sum_w': 0.0, 'sum_l': 0.0},
                'Historica / Contaminada': {'n': 0, 'w': 0, 'l': 0, 'sum_w': 0.0, 'sum_l': 0.0},
                'Manual (revision aparte)': {'n': 0, 'w': 0, 'l': 0, 'sum_w': 0.0, 'sum_l': 0.0},
            }
        try:
            gan = float(f.get('Ganancia $', 0) or 0)
        except Exception:
            gan = 0.0
        d = stats[est][cubo]
        d['n'] += 1
...
if ws_sim is not None and not senales.empty:
    for _, s in senales.iterrows():
        ...
        forzar_h = st.sidebar.checkbox("Forzar fuera de horario (quedara INVALIDA)", key="forzar_" + tk)
        if st.sidebar.button("COMPRAR " + tk, key="buy_" + tk):
            ahora = datetime.now(ZONA_NY)
            ok_h, msg_h = validar_horario_manual(str(s['Estrategia Cardona']), ahora)
            if not ok_h and not forzar_h:
                st.sidebar.error("Compra BLOQUEADA por horario: " + msg_h)
            else:
                if ok_h:
                    nota_base = 'compra manual | entrada valida (horario Cardona)'
                else:
                    nota_base = 'compra manual FORZADA (entrada INVALIDA: ' + msg_h + ')'
                ...
                for nota_lote in lotes:
                    ...
                    filas_sim.append({
                        ...
                        'Notas': nota_lote + ' | ' + nota_base, 'VI': '', 'DTE': dte, 'Break Even': be,
...
st.subheader("Estadisticas por estrategia (muestra limpia vs historica)")
if cerradas:
    stats = calcular_estadisticas(cerradas)
    rows_stats = []
    for est, cubos in stats.items():
        for cubo, d in cubos.items():
            if d['n'] > 0:
                wr = d['w'] / d['n']
                aw = d['sum_w'] / d['w'] if d['w'] > 0 else 0.0
                al = d['sum_l'] / d['l'] if d['l'] > 0 else 0.0
                em = wr * aw - (1.0 - wr) * al
                rows_stats.append({
                    'Estrategia': est,
                    'Muestra': cubo,
                    ...
                    'Ganancia prom': f"{aw:+.2f}",
                    'Perdida prom': f"{al:.2f}",
                    'EM': f"{em:+.2f}",
                    'Aviso': '' if d['n'] >= 10 else 'n<10: no juzgar aun',
                })
    if rows_stats:
        st.dataframe(pd.DataFrame(rows_stats), width='stretch', hide_index=True)
        st.caption("Regla del curso: consistencia de 10 trades antes de juzgar una estrategia. EM (expectativa matematica) > 0 = estrategia rentable a largo plazo.")
else:
